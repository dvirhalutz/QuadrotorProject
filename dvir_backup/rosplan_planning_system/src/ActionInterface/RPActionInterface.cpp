...
namespace KCL_rosplan {
	...
	void RPActionInterface::runActionInterface() {
		...
		ros::NodeHandle nh("~");
		//ROS_INFO("(1)DVIR: Im in ActionIterface.cpp -> runActionInterface()");
		// set action name
        ...
        spinner.start();
		//ROS_INFO("(2)DVIR: Im in ActionIterface.cpp -> runActionInterface()");

		ROS_INFO("KCL: (%s) Ready to receive", params.name.c_str());

		while(ros::ok()) {
			pddl_action_parameters_pub.publish(params);
			//ROS_INFO("(3)DVIR: Im in ActionIterface.cpp -> runActionInterface(), message is %s",params.name.c_str());
			loopRate.sleep();
		}
	}
	...
	//ROS_INFO("(4)DVIR: Im in ActionIterface.cpp -> dispatchCallback()");
		// check action name
		...
		//ROS_INFO("(5)DVIR: Im in ActionIterface.cpp -> dispatchCallback()- calling concreteCallBack");
